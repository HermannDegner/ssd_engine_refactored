"""
ニュートンのゆりかご - SSDCore直結版
Newton's Cradle with Direct SSDCoreEngine (No HumanAgent)

【最適化】
- HumanAgentを経由せず、SSDCoreEngineに直接接続
- 物理系なので社会的解釈層(HumanLayer)は不要
- より高速な物理演算

作成日: 2025年11月7日
バージョン: 1.0 (Direct Core)
"""
...
from ssd_core_engine import SSDCoreState
...
class BallDirect:
    ...
    def apply_impact(self, impact_velocity: float, current_time: float):
        ...
        # 直接E配列に追加（HumanLayerを経由しない）
        # [PHYSICAL, BASE, CORE, UPPER]
        pressure_vector = np.array([
            impact_magnitude * 2.0,  # PHYSICAL
            impact_magnitude * 1.5,  # BASE
            impact_magnitude * 0.5,  # CORE
            impact_magnitude * 0.2   # UPPER
        ])
        
        # SSDCoreStateに直接圧力を追加
        self.ssd_state.E += pressure_vector * 0.01  # dt=0.01相当
        
        # κ更新（慣性学習）
        self.ssd_state.kappa *= 0.99  # 減衰
        
        # E蓄積による減衰
        E_mean = np.mean(self.ssd_state.E)
        self.damping_factor = E_mean * 0.01
...
class NewtonsCradleDirectCore:
    ...
    def __init__(self, n_balls: int = 5, spacing: float = 1.0,
        ...
        for i in range(n_balls):
            ball = BallDirect(ball_id=i, initial_position=0.0, mass=self.mass)
    ...
    def detect_collisions(self):
        ...
        for i in range(self.n_balls - 1):
            ...
            # 支点からの水平位置
            angle1 = ball1.position / self.string_length
            angle2 = ball2.position / self.string_length
            
            support_x1 = (i - self.n_balls/2) * self.spacing
            support_x2 = (i + 1 - self.n_balls/2) * self.spacing
            
            x1 = support_x1 + self.string_length * np.sin(angle1)
            x2 = support_x2 + self.string_length * np.sin(angle2)
...
class DirectCoreVisualizer:
    """SSDCore直結版ビジュアライザー"""
    
    def __init__(self, cradle: NewtonsCradleDirectCore):
        self.cradle = cradle
        
        # Figure作成
        self.fig = plt.figure(figsize=(16, 10))
        self.fig.suptitle("Newton's Cradle - Direct SSDCore (No HumanAgent)", 
        ...
        self.ax_ssd = self.fig.add_subplot(gs[1, 1])
    ...
    def init_pendulum_plot(self):
        ...
        self.ax_pendulum.set_title('Physical Simulation (Direct SSDCore)', 
    ...
    def draw_pendulums(self):
        ...
        for i in range(self.cradle.n_balls):
            ...
            # 球
            recent_collision = (self.cradle.current_time - ball.last_collision_time) < 0.1
    ...
    def draw_ssd_state(self):
        """SSD状態プロット"""
        self.ax_ssd.clear()
        self.ax_ssd.set_title('SSDCore State: E Direct', fontweight='bold', fontsize=11)
        ...
        # E蓄積（SSDCore直接）
        E_physical = [b.ssd_state.E[0] for b in self.cradle.balls]
        E_base = [b.ssd_state.E[1] for b in self.cradle.balls]
        E_core = [b.ssd_state.E[2] for b in self.cradle.balls]
        E_upper = [b.ssd_state.E[3] for b in self.cradle.balls]
        
        width = 0.2
        self.ax_ssd.bar([i - 1.5*width for i in ball_ids], E_physical, width, 
                       label='E_PHYSICAL', alpha=0.7, color='blue')
        self.ax_ssd.bar([i - 0.5*width for i in ball_ids], E_base, width, 
                       label='E_BASE', alpha=0.7, color='orange')
        self.ax_ssd.bar([i + 0.5*width for i in ball_ids], E_core, width, 
                       label='E_CORE', alpha=0.7, color='red')
        self.ax_ssd.bar([i + 1.5*width for i in ball_ids], E_upper, width, 
                       label='E_UPPER', alpha=0.7, color='purple')
        
        self.ax_ssd.legend(fontsize=8)
        self.ax_ssd.grid(True, alpha=0.3)
        
        # 統計情報
        total_collisions = sum(b.collision_count for b in self.cradle.balls)
        total_E = sum(np.sum(b.ssd_state.E) for b in self.cradle.balls)
    ...
    def animate(self, frames: int = 1000, interval: int = 20):
        """アニメーション開始"""
        print(f"\nDirect SSDCore アニメーション開始: {frames}フレーム")
        print("HumanAgentを経由せず、SSDCoreに直接接続")
...
def demo_direct_core():
    """Direct SSDCore デモ"""
    print("="*70)
    print("Newton's Cradle - Direct SSDCore (No HumanAgent)")
    print("="*70)
    print("\nHumanLayerをバイパスして直接SSDCoreに接続")
    print("物理系に最適化された高速バージョン\n")
    
    cradle = NewtonsCradleDirectCore(
        n_balls=5,
        spacing=1.0,
        string_length=2.0,
    ...
    viz = DirectCoreVisualizer(cradle)
...
if __name__ == "__main__":
    ...
    print("="*70)
    print("\n💡 最適化ポイント:")
    print("  - HumanAgent/HumanLayerをバイパス")
    print("  - SSDCoreEngine.state.E に直接アクセス")
    print("  - 社会的解釈層を削除して高速化")
